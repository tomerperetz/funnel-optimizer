...
def update_env_file(token: str, page_id: Optional[str] = None) -> None:
    """Update .env file with new token.

    Args:
        token: New access token
        page_id: Optional page ID to set as default
    """
    ...
    for line in lines:
        if line.startswith("FO_META_ACCESS_TOKEN="):
            new_lines.append(f"FO_META_ACCESS_TOKEN={token}")
            token_updated = True
        elif page_id and line.startswith("FO_META_PAGE_ID="):
            new_lines.append(f"FO_META_PAGE_ID={page_id}")
            page_updated = True
        else:
    ...
    if page_id and not page_updated:
        new_lines.append(f"FO_META_PAGE_ID={page_id}")
